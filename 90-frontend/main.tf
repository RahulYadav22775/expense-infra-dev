...
resource "aws_autoscaling_group" "frontend" {
  ...
  desired_capacity          = 2 #strting of auto scaling group
 ...
 # if instances are not healthy within 15min , then autoscale will delete that instance
  timeouts {
    delete = "15m"
  }
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  priority     = 100  #low priority number evaluate first
